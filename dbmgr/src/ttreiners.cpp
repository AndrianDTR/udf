...
std::string CTrainersTable::GetTableName()
{
	return TABLE;
}

std::string CTrainersTable::GetFilterString(const tDATA* const filter)
{
...
long CTrainersTable::Find(tTableMap& data, const tDATA* const filter)
{
	long res = UDF_E_FAIL;
	...
	do
	{
		...
		while( qRes && qRes->next())
		{
			...
			el.clubId = qRes->getUInt("club_id");
			el.name = qRes->getString("name");
			el.bd = str2date(qRes->getString("bd"));
