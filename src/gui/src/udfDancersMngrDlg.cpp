...
void udfDancersMngrDlg::OnDancerSelect( wxCommandEvent& event )
{
	do
	{
		...
		m_staticRaiting->SetLabel(STR_FORMAT(_("%d"), data.raiting));
...
int udfDancersMngrDlg::GetSelectedLigue()
{
	...
	do
	{
		...
		if(-1 == res)
		{
			ShowWarning(STR_FORMAT(STR_NOT_IN_DB, STR_LIGUE));
			break;
		}
	}while(0);
...
int udfDancersMngrDlg::GetSelectedTrainer()
{
	...
	do
	{
		...
		if(-1 == res)
		{
			ShowWarning(STR_FORMAT(STR_NOT_IN_DB, STR_TRAINER));
			break;
		}
	}while(0);
...
int udfDancersMngrDlg::GetSelectedGender()
{
	...
	do
	{
		...
		if(-1 == res)
		{
			ShowWarning(STR_FORMAT(STR_NOT_IN_DB, STR_GENDER));
			break;
		}
	}while(0);
